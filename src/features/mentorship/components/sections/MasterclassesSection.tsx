...
import { ArrowRight, Sparkles, Clock, Users, Star } from "lucide-react";
import { MASTERCLASSES, GRADIENT_COLORS } from "@/features/index";

export default function MasterclassesSection() {
    return (
        <section className="py-16 px-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 reveal-on-scroll text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-18 mb-12">
                    {MASTERCLASSES.map((mc, i) => (
                        <div
                            key={i}
                            className="group relative w-72 rounded-[28px] overflow-hidden border border-[#ece7e2] shadow-lg hover:shadow-2xl transition-all duration-700 cursor-pointer hover:-translate-y-2 reveal-on-scroll mx-auto"
                            style={{
                                animationDelay: `${i * 100}ms`,
                                background: `linear-gradient(135deg, ${GRADIENT_COLORS[i % GRADIENT_COLORS.length].start} 0%, ${GRADIENT_COLORS[i % GRADIENT_COLORS.length].end} 100%)`,
                            }}
                        >
